          ...
          <div className='flex gap-3 sm:justify-between xm:justify-between  sm:flex-row xm:flex-col'>
            ...
            <div>
              <select onChange={(e) => setTimeStatus(e.target.value)} value={timeStatus} className="border py-3 px-3 rounded font-bold">
                ...
                <option className='font-bold' value={'جديد'}>الأحدث</option>
                <option className='font-bold' value={'قديم'}>الأقدم</option>
              </select>
            </div>
          </div>
                  ...
                  <tr key={index} className="border border-gray-300">
                    ...
                    <td className={`md:hidden sm:hidden text-center xm:hidden xl:table-cell flex `}>
                      <div className={`${order.status === 'جديد' ?
                         'text-blue-400 bg-blue-100' : order.status === "قيد المراجعة" ?
                          'text-orange-400 bg-orange-100' : order.status === 'ملغى' ? 'text-red-400 bg-red-100' :
                           'text-green-500 bg-green-100'}  py-2 w-38 rounded-sm  font-bold inline-block cursor-pointer relative `}><h1>{order.status}   {order.status === "تم التوصيل" ? <TbTruckDelivery className='absolute left-1 top-[10px] text-xl' />:
